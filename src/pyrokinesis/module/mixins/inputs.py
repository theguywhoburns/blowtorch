...
class InputMixin:
    ...
    _pk_input_entries: ClassVar[tuple[tuple[str, InputSpec], ...]] = (
        ("x", InputSpec(primary=True)),
    )
    _pk_input_names: ClassVar[tuple[str, ...]] = ("x",)
    _pk_input_specs: ClassVar[tuple[InputSpec, ...]] = (InputSpec(primary=True),)
    _pk_primary_input_index: ClassVar[int] = 0
    ...
    def _pk_canonicalize_inputs(
        ...
        if isinstance(inputs, dict):
            missing = [
                name for name in self._pk_input_names if name not in inputs
            ]
            ...
            return tuple(inputs[name] for name in self._pk_input_names)

        if isinstance(inputs, (tuple, list)):
            if len(inputs) != len(self._pk_input_names):
                raise ValueError(
                    f"{type(self).__name__} expects {len(self._pk_input_names)} "
        ...
        if isinstance(inputs, Tensor):
            if len(self._pk_input_names) != 1:
                raise ValueError(
                    f"{type(self).__name__} expects "
                    f"{len(self._pk_input_names)} inputs, got a single tensor"
                )
