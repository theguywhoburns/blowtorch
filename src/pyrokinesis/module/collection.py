...
def collect_specs(cls: type[PyroModule]) -> tuple[tuple[str, Spec], ...]:
    """
...
def collect_inputs(cls: type[PyroModule]) -> tuple[tuple[str, InputSpec], ...]:
    """
...
def check_input_namespace_collisions(cls: type[PyroModule]) -> None:
    """
    Input names share the module namespace with params, constants, states,
    and outputs; reject collisions with a clear message.
    """
    param_names = set(cls._pk_param_specs)
    constant_names = set(cls._pk_constant_specs)
    input_names = set(cls._pk_input_names)
    spec_names = set(name for name, _ in cls._pk_spec_entries)
...
def collect_metadata(cls: type[PyroModule]) -> None:
    """
    Gather all ``_pk_*`` metadata on a freshly created subclass.
    ...
    (
        cls._pk_param_specs,
        cls._pk_param_annotations,
        cls._pk_constant_specs,
        cls._pk_constant_annotations,
    ) = collect_params(cls)
    cls._pk_spec_entries = collect_specs(cls)

    cls._pk_input_entries = collect_inputs(cls)
    cls._pk_input_names = tuple(
        name for name, _ in cls._pk_input_entries
    )
    cls._pk_input_specs = tuple(
        spec for _, spec in cls._pk_input_entries
    )

    primary_indices = [
        i
        for i, (_, spec) in enumerate(cls._pk_input_entries)
        if spec.primary
    ]
    ...
    if len(primary_indices) > 1:
        raise TypeError(
            ...
            f"{[cls._pk_input_names[i] for i in primary_indices]}. "
            f"At most one Input may set primary=True."
        )

    cls._pk_primary_input_index = primary_indices[0] if primary_indices else 0

    check_input_namespace_collisions(cls)

    cls._pk_output_names = tuple(
        name
        for name, spec in cls._pk_spec_entries
        if isinstance(spec, OutputSpec)
    )

    cls._pk_state_names = tuple(
        name
        for name, spec in cls._pk_spec_entries
        if isinstance(spec, StateSpec)
    )

    cls._pk_output_specs = tuple(
        spec
        for _, spec in cls._pk_spec_entries
        if isinstance(spec, OutputSpec)
    )

    cls._pk_state_specs = tuple(
        spec
        for _, spec in cls._pk_spec_entries
        if isinstance(spec, StateSpec)
    )
...
def generate_signature(cls: type[PyroModule]) -> None:
    ...
    for name, spec in cls._pk_constant_specs.items():
        ann = (
            spec.dtype
            if spec.dtype is not None
            else cls._pk_constant_annotations.get(name, Any)
        )
        sig_params.append(
            inspect.Parameter(
                name,
