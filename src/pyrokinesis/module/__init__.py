...
class PyroModule(
    ...
    output through frozen ``_pk_hook_post__*`` chain entries, which run at the end of
    ``_pk_forward_explicit`` before the output is returned or stored;
    ...
    Declaring inputs:
      ...
      or with an ``InputSpec`` value for extra semantics::
          ...
          class Inputs:
              x: Tensor = PyroModule.Input(primary=True)
              inh: Tensor
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._pk_allocated: bool = False
        self._pk_compiled_sequence: Optional[Callable[..., Any]] = None
