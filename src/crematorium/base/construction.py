...
def build_params(
    ...
    for name, spec in module._bt_param_specs.items():
    ...
    for name, spec in module._bt_constant_specs.items():
        value = kwargs.pop(name, spec.default)
        value_orig = value
...
def install_constrained(module: crematoriumModule) -> None:
    ...
    param_names = tuple(module._bt_param_specs.keys())

    for i, (name, constraint) in enumerate(
        zip(param_names, module._bt_constraints)
    ):
    ...
    setattr(
        module,
        "_bt_constrained_fn",
        types.MethodType(ns["_bt_constrained"], module),
    )
