...
# surface declared by _ValidationHost; CrModule and SequenceScanMixin
...
def check_hidden_input_shape(
    ...
    for name, spec in zip(module._cr_state_names, module._cr_state_specs, strict=True):
        ref = module._buffers.get(name)
        expected = module._cr_spec_shape(spec, inputs)
...
def check_input_dtypes(
    ...
    for name, spec, x in zip(
        module._cr_input_names, module._cr_input_specs, inputs, strict=True
    ):
        ...
        if isinstance(spec.dtype, torch.dtype):
            ...
            expected = _cr_floating_dtype(x.dtype)
...
def check_step_output(module: _ValidationHost, out: StepOutput) -> None:
    ...
    expected = len(module._cr_spec_entries)
