...
class StateMixin(InputMixin):
    ...
    _cr_spec_extensions: ClassVar[dict[str, Callable[..., Any]]] = {}

    _cr_input_names: ClassVar[tuple[str, ...]]
    _cr_primary_input_index: ClassVar[int]
    init_hidden: bool
    _cr_allocated: bool
    ...
    def _cr_process_spec_extensions(self) -> None:
        """
        ...
        metadata (``_cr_state_specs``, ``_cr_param_specs``) and on instance
        parameters already being registered. Then runs the frozen
        ``_cr_hook_specs__*`` chain (e.g. reset-fn install); hooks are
        collected once per class, never resolved per call.
        """
        for i, spec in enumerate(self._cr_state_specs):
            for key, value in spec.extras.items():
                handler = self._cr_spec_extensions.get(key)
                if handler is not None:
                    handler(self, i, spec, value)
        for fn in self._cr_hook_specs_steps:
            fn(self)

    def _cr_resolve_default(
        self,
    ...
    def _cr_spec_shape(self, spec: Spec, inputs: tuple[Tensor, ...]) -> tuple[int, ...]:
        shape = getattr(spec, "shape", "input")

        if shape is None or shape == "input":
            return tuple(inputs[self._cr_primary_input_index].shape)

        if isinstance(shape, str):
            if shape not in self._cr_input_names:
                raise ValueError(
                    f"{type(self).__name__} StateSpec(shape={shape!r}) refers "
                    f"to an unknown input; declared inputs are "
                    f"{self._cr_input_names}"
                )

            return tuple(inputs[self._cr_input_names.index(shape)].shape)

        assert isinstance(shape, tuple)
        return shape

    def _cr_explicit_state_dtype(
        self,
        dtype: Optional[torch.dtype],
    ) -> Optional[torch.dtype]:
        if dtype is None:
            return None
        return _cr_floating_dtype(dtype)
    ...
    def _cr_alloc_hidden(self, inputs: tuple[Tensor, ...]) -> None:
        ...
        # the first store (_cr_store_hidden_outputs / the sequence-buffer
        ...
        for name, spec in self._cr_spec_entries:
            if not isinstance(spec, StateSpec):
                continue

            self._buffers[name] = torch.full(
                self._cr_spec_shape(spec, inputs),
    ...
    def allocate_like(
        ...
        *inputs: InputTensor,
        ...
        if self.init_hidden and not self._cr_allocated:
            if len(inputs) == 1:
                canonical = self._cr_canonicalize_inputs(inputs[0])
            else:
            ...
            self._cr_alloc_hidden(canonical)

        return self

    def _cr_store_hidden_outputs(self, out: StepOutput) -> None:
        for (name, spec), t in zip(self._cr_spec_entries, out, strict=True):
            if not spec.differentiable:
                t = t.detach()

            # Buffers already exist after _cr_alloc_hidden (states) or are
    ...
    def detach(self) -> None:
        ...
        for name, _ in self._cr_spec_entries:
            t = self._buffers.get(name, None)
