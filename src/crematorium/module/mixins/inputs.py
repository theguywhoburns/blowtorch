...
class InputMixin:
    ...
    _cr_input_entries: ClassVar[tuple[tuple[str, InputSpec], ...]] = (
        ("x", InputSpec(primary=True)),
    )
    _cr_input_names: ClassVar[tuple[str, ...]] = ("x",)
    _cr_input_specs: ClassVar[tuple[InputSpec, ...]] = (InputSpec(primary=True),)
    _cr_primary_input_index: ClassVar[int] = 0

    def _cr_canonicalize_inputs(
        self,
        inputs: InputTensor,
    ) -> tuple[Tensor, ...]:
        """
        ...
        if isinstance(inputs, dict):
            ...
            return tuple(inputs[name] for name in self._cr_input_names)

        if isinstance(inputs, (tuple, list)):
            if len(inputs) != len(self._cr_input_names):
                raise ValueError(
                    f"{type(self).__name__} expects {len(self._cr_input_names)} "
        ...
        if isinstance(inputs, Tensor):
            if len(self._cr_input_names) != 1:
                raise ValueError(
                    f"{type(self).__name__} expects "
                    f"{len(self._cr_input_names)} inputs, got a single tensor"
                )
