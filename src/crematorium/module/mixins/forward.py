...
# for _cr_fwd_explicit instead of redeclaring it as a stub.
...
class ForwardMixin(StateMixin):
    ...
    def _cr_hidden_step(self, inputs: tuple[Tensor, ...]) -> StepOutput:
        """
        ...
        out = self._cr_forward_explicit(inputs, state)

        self._cr_store_hidden_outputs(out)
        return out

    def _cr_forward_hidden(self, inputs: tuple[Tensor, ...]) -> Tensor | StepOutput:
        if not self._cr_allocated:
            self._cr_alloc_hidden(inputs)
        elif is_validating(self):
            check_hidden_input_shape(self, inputs)

        out = self._cr_hidden_step(inputs)
    ...
    def _cr_post_step(self, out: StepOutput) -> StepOutput:
        """
        Driver applied to the raw output of ``_step`` before it is returned
        or stored.

        Runs the frozen ``_cr_hook_post__*`` chain (child-first, collected
        once per class in ``CrModule.__init_subclass__``); empty for plain
        ...
        validated mode.
        """
        for fn in self._cr_hook_post_steps:
            out = fn(self, out)
        if is_validating(self):
            check_step_output(self, out)
        return out

    def _cr_forward_explicit(
        self,
        inputs: tuple[Tensor, ...],
        state: tuple[Tensor, ...],
    ) -> StepOutput:
        ...
        # validate flag.
        expected_inputs = len(self._cr_input_names)
        ...
        expected_state = len(self._cr_state_names)
    ...
    def forward(
        ...
        *state: Tensor,
        ...
        if self.init_hidden:
            ...
            return self._cr_forward_hidden(inputs)

        return self._cr_forward_explicit(inputs, tuple(state))
